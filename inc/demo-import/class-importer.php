...
class Importer {
	...
	/**
	 * The instance of the Demo Import class.
	 *
	 * @var object
	 */
	private $bnmbt_importer;
	...
	public function __construct( $importer_options = array(), $logger = null ) {
		...
		$this->importer = new WXRImporter2( $importer_options );
		...
		$this->bnmbt_importer = DemoImporter::get_instance();
	}
	...
	public function import_content( $import_file_path ) {
		...
		if ( strpos( ini_get( 'disable_functions' ), 'set_time_limit' ) === false ) {
			set_time_limit( apply_filters( 'bnmbt_importer_set_time_limit_for_demo_data_import', 300 ) );
		}
		...
		if ( ! apply_filters( 'bnmbt_importer_regenerate_thumbnails_in_content_import', true ) ) {
	...
	private function set_current_importer_data() {
		$data = array_merge( $this->bnmbt_importer->get_current_importer_data(), $this->get_importer_data() );

		Helpers::set_bnmbt_import_data_transient( $data );
	}
}