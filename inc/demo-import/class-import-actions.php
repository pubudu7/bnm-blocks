...
class ImportActions {
	...
	public function register_hooks() {
		...
		// Customizer import.
		add_action( 'bnmbt_importer_customizer_import_execution', array( $this, 'customizer_import' ), 10, 1 );

		// After full import action.
		add_action( 'bnmbt_importer_after_all_import_execution', array( $this, 'after_import_action' ), 10, 3 );

		// Special widget import cases.
		if ( apply_filters( 'bnmbt_importer_enable_custom_menu_widget_ids_fix', true ) ) {
			add_action( 'bnmbt_importer_widget_settings_array', array( $this, 'fix_custom_menu_widget_ids' ) );
		}
	}
	...
	public function fix_custom_menu_widget_ids( $widget ) {
		...
		$content_import_data = $demo_importer->importer->get_importer_data();
	 ...
	 * @param array $import_files          The filtered import files defined in `bnmbt_import_files` filter.
	...
	/**
	 * Execute the action: 'bnmbt_importer_before_content_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `bnmbt_import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function before_content_import_action( $selected_import_files, $import_files, $selected_index ) {
		$this->do_import_action( 'bnmbt_importer_before_content_import', $import_files[ $selected_index ] );
	}


	/**
	 * Execute the action: 'bnmbt_importer_before_widgets_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `bnmbt_import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function before_widget_import_action( $selected_import_files, $import_files, $selected_index ) {
		$this->do_import_action( 'bnmbt_importer_before_widgets_import', $import_files[ $selected_index ] );
	}


	/**
	 * Execute the action: 'bnmbt_importer_after_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `bnmbt_import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function after_import_action( $selected_import_files, $import_files, $selected_index ) {
		$this->do_import_action( 'bnmbt_importer_after_import', $import_files[ $selected_index ] );
	}
	 ...
	 * @param array  $selected_import The data of selected import from `bnmbt_import_files` filter.
	 */
	private function do_import_action( $action, $selected_import ) {
		if ( false !== has_action( $action ) ) {
			$demoimporter  = DemoImporter::get_instance();
			$log_file_path = $demoimporter->get_log_file_path();
